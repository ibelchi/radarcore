...
class SwingScorer(BaseBucketScorer):
    def score(self, hist_data: pd.DataFrame, metrics: dict) -> dict:
        ...
        # Days since minimum (approx 60 days window)
        idx_min = hist_data['Low'].tail(60).idxmin()
        try:
            days_since_low = len(hist_data) - hist_data.index.get_loc(idx_min) - 1
        except:
            days_since_low = 0
            
        if 5 <= days_since_low <= 30:
            total_score += 25
        elif days_since_low > 30:
            total_score += 10
            
        # Volume last segment (approx last 5 days vs 20)
        ...
        reasoning = f"Swing: {upside_ath3y:.1f}% upside, {days_since_low} days since low, volume {vol_str}"
        ...
        return {
            ...
            "key_metrics": {
                ...
                "volume_ratio": volum_ultims_5d / volum_mitjana_20d if volum_mitjana_20d > 0 else 1
            }
        }
